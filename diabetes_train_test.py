# -*- coding: utf-8 -*-
"""
Created on Wed Apr 15 10:12:07 2020

@author: Peiyu Wang
"""

"""
divide the doctor notes (with icd-10 code in classes kept above) into training set and testing set with
ratio 7:3. For each note get rid of the stopwords and digits/numbers because they do not carry useful 
information
"""
...
def get_non_diabetes_training_testing(doctor_notes_exp,class_list,training_set_num_by_class):
...
def get_diabetes_training_testing(doctor_notes_exp,diabetes_df,diabetes_list):
    training_df=pd.DataFrame()
    testing_df=pd.DataFrame()
...
def diabetes_or_not_resampling(other_training_df,diabetes_training_df):
    resampled_df = resample(diabetes_training_df, 
                            replace=True,     # sample with replacement
                            n_samples=len(other_training_df),    # to match majority class
                            random_state=4)
    training_df=pd.concat([other_training_df, resampled_df])
    training_df=shuffle(training_df,random_state=4)
    return training_df

def diabetes_or_not_testing(diabetes_testing,non_diabetes_testing):
    testing_df=pd.concat([non_diabetes_testing, diabetes_testing])
    testing_df=shuffle(testing_df,random_state=4)
    return testing_df
...
def diabetes_label_processing(training_df,testing_df,diabetes_list):
    Y_train=training_df['icd_code_first_digits']
    Y_test=testing_df['icd_code_first_digits']
    Y_train=Y_train.apply(lambda x: 1 if x in diabetes_list else 0)
    Y_test = Y_test.apply(lambda x: 1 if x in diabetes_list else 0)
